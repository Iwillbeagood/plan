...
sealed interface SavingsType {
    ...
    companion object {
        ...
        val interestTypes: List<SavingsType> = allTypes.filter { it is 정기예금 || it is 적금 || it is 청약저축 || it is 보험저축 }

        val periodTypes: List<SavingsType> = allTypes.filter { it is 정기예금 || it is 적금 || it is 보험저축 }
    }
}