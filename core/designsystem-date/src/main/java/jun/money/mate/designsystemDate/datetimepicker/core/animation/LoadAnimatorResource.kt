...
internal fun AnticipateEasing(tension: Float) = Easing { x -> x * x * ((tension + 1) * x - tension) }

internal fun AnticipateOvershootEasing(
    tension: Float,
    extraTension: Float,
): Easing = AnticipateOvershootInterpolator(tension, extraTension).toEasing()
...
internal fun OvershootEasing(tension: Float) = Easing { x -> (x - 1f).let { t -> t * t * ((tension + 1f) * t + tension) + 1f } }
