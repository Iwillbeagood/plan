...
import jun.money.mate.designsystemDate.datetimepicker.models.CalendarData
import jun.money.mate.designsystemDate.datetimepicker.models.CalendarDateData
import jun.money.mate.designsystemDate.datetimepicker.models.CalendarMonthData
...
import jun.money.mate.designsystemDate.datetimepicker.models.CalendarViewType
 ...
 */
@RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
fun LocalDate.jumpPrev(config: CalendarConfig): LocalDate =
    when (config.style) {
 ...
 */
@RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
fun LocalDate.jumpNext(config: CalendarConfig): LocalDate =
    when (config.style) {
...
internal val CalendarSelection.datesValue: Array<LocalDate>
    ...
    get() {
        val value =
            if (this is CalendarSelection.Dates) {
...
internal fun calcCalendarData(
    ...
    val weekDays: List<List<Pair<CalendarViewType, Any>>> =
        chunkedDays.map { week ->
            if (config.displayCalendarWeeks) {
                val newWeek =
                    week.toMutableList().apply {
